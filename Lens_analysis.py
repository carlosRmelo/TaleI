...
def run(result_path, data_path, phase_name, alpha=None, Re=None):
    ...
    distance = CM.cosmology.angular_diameter_distance(z=CM.Lens_model.z_l)
    R_kpc = ( (R*u.arcsec * distance ).to(
                        u.kpc,u.dimensionless_angles()) ).value # R in kpc


        # Get the radial mass of stars and DM within rad
    MMstar = float ( mge_radial_mass(CM.Lens_model.surf_lum * CM.Lens_model.ml_model, 
                                CM.Lens_model.sigma_lum, CM.Lens_model.qobs_lum,
                                CM.Lens_model.inc, R, CM.Lens_model.distance) )
