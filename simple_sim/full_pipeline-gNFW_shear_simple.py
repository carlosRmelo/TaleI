...
# scalar mass-to-light ratio and slip parameter. 
# 
# Date: 16/03/2023
# 
import shutil
...
def run(data_path, ncores):
    ...
    vrms_err = vrms_err[ii]
        #MGE components
    surf, sigma, qObs = np.load('%s/star_mge.npy'%data_path,  allow_pickle=True)
    ...
    fits.writeto('{}/noise_scaled.fits'.format(data_path), data=noise, 
        ...
        # re-size the dataset
    """
    Only use these lines with you want to cut your image in a small FoV
    In this case, also change the name of the image when reading the data bellow
    """
    # header = fits.open('{}/image.fits'.format(data_path))[0].header
    # for name in ["image", "noise_scaled"]:
    #         # Open large image
    #     data_large_stamp = al.Array2D.from_fits(
    #         file_path="{}/{}.fits".format(data_path,name), pixel_scales=header["PIXSCALE"]
    #     )
    #         # Cut same size as the normal dataset
    #     data_large_stamp_trimmed = al.preprocess.array_with_new_shape(
    #         array=data_large_stamp, new_shape=(263, 263)
    #     )
    #     if name == "noise_scaled":
    #         header = None
    #     fits.writeto('{}/{}_cut.fits'.format(data_path,name),
    #                     data=data_large_stamp_trimmed.native, header=header,
    #                     overwrite=True) # save scaled noise
    ...
    reff, reff_maj, eps_e, lum_tot = mge_half_light_isophote(surf, sigma, qObs, log["distance"])
    ...
    parsDic = {
                ...
                "rs":      p.UniformPrior(1e-2, 150.),
                "qDM":     1.0,
                "slope":   p.UniformPrior(0.5, 2.0),
                "mag_shear":p.UniformPrior(0.0, 1.0),
                "phi_shear":p.UniformPrior(0.0, 270.0),
                "gamma":   1.0,
                }
    ...
    output_path = data_path.split("/")[1]+"/model1/normal/shear/dyLens/"
    ph1 = Ph1Model(CombinedModel=CM, output_path=output_path)

        ## Update the non-linear sampler to save some time
    ph1.config_non_linear(n_cores=ncores, nlive=1500)
    ...
    for name in ["/phase1", "/phase2", "/phase3", "/phase4"]:
        c = shutil.make_archive(base_name=output_path+name, 
                                format="xztar", root_dir=output_path+name)
