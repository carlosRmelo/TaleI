...
# Here we use the fiducial model to derive such quantities
...
def run(result_path, data_path, JAM=None, dyLens=None, Lens=None,
            alpha=None, Re=None, phase_name=None, ncores=40):
    ...
    if Re:
        analysis_path = result_path+"/Fiducial/Analysis_Re/"  # path where the analysis will be saved
    else:
        analysis_path = result_path+"/Fiducial/Analysis_{:.1f}Reff/".format(alpha) # path where the analysis will be saved
    ...
    if JAM or dyLens:
            # 3D Quantities
        MMstar  = A.Fiducial_stellarMass_3D(R=R)
        MMdm    = A.Fiducial_dmMass_3D(R=R)
        MMbh    = 0.0            #TODO: Fix it in a new version of Analysis class
        MMtotal = MMstar + MMdm
        Mfdm    = MMdm / MMtotal

            # 2D Quantities
        projMMstar  = A.Fiducial_stellarMass_2D(R=R)
        projMMdm    = A.Fiducial_dmMass_2D(R=R)
        projMMtotal = projMMstar + projMMdm
        projMfdm    = projMMdm / projMMtotal

    else:
            # 3D Quantities
        MMstar = MMdm = MMtotal = Mfdm = 0.0  # Lens only is not sensitive to them.
        MMbh = 0.0
    ...
    if JAM or dyLens:
        ...
        dstar  = np.asarray( A.Fiducial_stellarMass3D_density(
                                                    radii=radii, 
                                                    save_path=analysis_densities)
                        )
        ddm    = np.asarray( A.Fiducial_dmMass3D_density(
                                                radii=radii,
                                                save_path=analysis_densities)
                        )
        dtotal = dstar + ddm
            ...
            # Plot stellar
        _plot_density(rho_star, dstar,
                r_star, reff_pc, Rmax_data_pc, Re_data_pc,
                label_true=r"Stellar$_{\rm true}$",
                label_model=r"Stellar$_{\rm model}$",
                save_path=analysis_densities, save_name="stellar_density")

            # Plot dm
        _plot_density(rho_dm, ddm, 
                r_dm, reff_pc, Rmax_data_pc, Re_data_pc,
            ...
            # Plots total
        _plot_density(rho_star + rho_dm, dtotal, 
                r_dm, reff_pc, Rmax_data_pc, Re_data_pc,
